...
Hermes state is stored under the persistent AMP instance path `/AMP/.hermes`, including configuration, credentials, sessions, skills, memory, logs, and cron jobs.
...
4. Complete the initial **Update** action. It creates `/AMP/.hermes/venv`, checks PyPI, and installs `hermes-agent[all]`; the output reports the installed version.
5. Start the instance and open the allocated **Hermes Web Dashboard** port. Log in, then use the dashboard's **API Keys**, **Models**, **Config**, **Channels**, and **Cron** pages to complete configuration.
...
Restart the AMP instance after a successful update. The persistent `/AMP/.hermes` data directory is not replaced by an application update.

### Cron

The running gateway owns the scheduler. After a model/provider is configured, create and manage recurring jobs through the dashboard **Cron** page, the embedded browser chat, or `hermes cron` in the instance console. Scheduled jobs persist at `/AMP/.hermes/cron/`.
